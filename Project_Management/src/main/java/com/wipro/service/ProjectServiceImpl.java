...
public class ProjectServiceImpl implements ProjectService {
	...
	public List<APIResponseDto> getProjectById(Long pid) throws ProjectIdNotFoundException
    {
		...
		/*
		 * Project savedProject = repository.findById(pid).orElseThrow(()->new
		 * RuntimeException("id not found"));
		 * 
		 * 
		 * List<TaskDto> Dto = apiClient.getAllTasksByPid(pid);
		 * 
		 * APIResponseDto apidto = new
		 * APIResponseDto(modelMapper.map(savedProject,ProjectDto.class),Dto);
		 * 
		 * List<APIResponseDto> apilist = List.of(apidto); return apilist;
		 */
